...
public class TestsType_SearchCityNameAndDateTest extends Test_BaseTest {
	...
	public void searchtest() throws IOException {
		...
		Log.info("Start - testCase_SearchCityNameAndDateTest_searchtest");
		...
		for (int i = 1; i <= rc; i++) {
			...
			if (data.get("RunChoice").equalsIgnoreCase("run")) {
				...
				try {
					...
					extentTests.log(LogStatus.PASS,"filled details src,dest,date are"+src+" "+dest+" "+dd+"-"+mm+"-"+yyyy);
				...
				extentTests.log(LogStatus.PASS,"SearchCityNameAndDateTest_searchtest_"+src+" "+dest+" "+dd+"-"+mm+"-"+yyyy+" test ends");
